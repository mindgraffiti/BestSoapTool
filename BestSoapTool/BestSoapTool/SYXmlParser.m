...
-(void)startParser{
	...
	if (URL != nil && dataToParse == nil) {		
		...
		if ([NSData dataWithContentsOfURL:URL] == nil) {
			UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"Connection Lost" message:@"Network Connection Lost" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
			[alert show];
			theDataArray = nil;		
		}
		else {
